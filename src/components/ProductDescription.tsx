    ...
    if (!parsed.isList) {
      return (
        <div className={`mt-1.5 ${className}`}>
          {parsed.mainText && (
            <p className="text-xs sm:text-sm text-neutral-300 line-clamp-2 sm:line-clamp-3 leading-relaxed font-normal">
              {parsed.mainText}
            </p>
          )}
          ...
          {parsed.items.map((item, idx) => {
            ...
            return (
              <div key={idx} className="flex items-start gap-2 text-xs sm:text-sm text-neutral-200">
                {qty ? (
                  <span className="inline-flex items-center justify-center w-5 h-5 rounded bg-amber-500/20 text-amber-400 font-bold text-xs shrink-0 mt-0.5">
                    {qty}
                  </span>
                ) : (
