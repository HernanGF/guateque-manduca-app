...
export const ProductDescription: React.FC<ProductDescriptionProps> = ({
  ...
  if (mode === 'modal') {
    if (!parsed.isList) {
      return (
        <div className={`mt-2 ${className}`}>
          {parsed.mainText && (
            <p className="text-base sm:text-lg text-neutral-800 leading-relaxed font-medium whitespace-pre-line">
              {parsed.mainText}
            </p>
          )}

          {parsed.footerNote && (
            <div className="mt-3.5 pt-3 border-t border-neutral-200 flex items-center gap-2.5 text-sm sm:text-base text-neutral-800">
              <span className="text-sky-600 text-base shrink-0">❄️</span>
              <span className="italic font-semibold">{parsed.footerNote}</span>
            </div>
          )}
        </div>
      );
    ...
    return (
      <div className={`mt-3 bg-neutral-50 border border-neutral-200 rounded-2xl p-4 sm:p-5 space-y-3.5 ${className}`}>
        <div className="flex items-center justify-between pb-2.5 border-b border-neutral-200">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm sm:text-base font-bold text-neutral-900 uppercase tracking-wider">
              Contenido del Box
            </span>
          </div>
          {!isCatering && (
        ...
        <ul className="space-y-2">
          {parsed.items.map((item, idx) => {
            ...
            return (
              <li
                key={idx}
                className="flex items-start gap-3 p-3 rounded-xl bg-white border border-neutral-200 hover:border-neutral-300 transition-colors shadow-xs"
              >
                {qty ? (
                  <span
                  ...
                  <span className="w-2.5 h-2.5 rounded-full bg-amber-500 mt-2 shrink-0" />
                )}
                <span className="text-base font-semibold text-neutral-900 leading-snug pt-0.5">
                  {name}
                </span>
              </li>
            );
        ...
        {parsed.footerNote && (
          <div className="pt-2.5 border-t border-neutral-200 flex items-center gap-2.5 text-sm text-neutral-800">
            <span className="text-sky-600 text-base shrink-0">❄️</span>
            <span className="italic font-semibold leading-relaxed">{parsed.footerNote}</span>
          </div>
        )}
      </div>
    );
    ...
    if (!parsed.isList) {
      return (
        <div className={`mt-1.5 ${className}`}>
          {parsed.mainText && (
            <p className="text-xs sm:text-sm text-neutral-800 line-clamp-2 sm:line-clamp-3 leading-relaxed font-medium">
              {parsed.mainText}
            </p>
          )}

          {parsed.footerNote && (
            <div className="pt-1.5 border-t border-neutral-200 mt-2 flex items-center gap-1.5 text-xs sm:text-sm text-neutral-700 font-medium">
              <span className="text-sky-600 text-sm shrink-0">❄️</span>
      ...
      <div className={`mt-2 space-y-1.5 ${className}`}>
        <div className="space-y-1">
          {parsed.items.map((item, idx) => {
            ...
            return (
              <div key={idx} className="flex items-start gap-2 text-xs sm:text-sm text-neutral-800 font-medium">
                {qty ? (
                  <span className="inline-flex items-center justify-center w-5 h-5 rounded bg-amber-100 text-amber-900 border border-amber-300 font-bold text-xs shrink-0 mt-0.5">
                    {qty}
                  </span>
                ) : (
                  <span className="text-amber-600 font-black shrink-0 leading-tight">•</span>
                )}
        ...
        {parsed.footerNote && (
          <div className="pt-1.5 border-t border-neutral-200 mt-1.5 flex items-center gap-1.5 text-xs sm:text-sm text-neutral-700 font-medium">
            <span className="text-sky-600 text-sm shrink-0">❄️</span>
            <span className="italic truncate">{parsed.footerNote}</span>
          </div>
        )}
  ...
  if (mode === 'card-list') {
    if (!parsed.isList) {
      return (
        <div className={`mt-2 ${className}`}>
          {parsed.mainText && (
            <p className="text-sm sm:text-base text-neutral-800 line-clamp-3 leading-relaxed font-medium">
              {parsed.mainText}
            </p>
          )}

          {parsed.footerNote && (
            <div className="pt-2 border-t border-neutral-200 mt-2.5 flex items-center gap-2 text-xs sm:text-sm text-neutral-700 font-medium">
              <span className="text-sky-600 text-sm shrink-0">❄️</span>
      ...
      <div className={`mt-2.5 space-y-1.5 ${className}`}>
        <div className="space-y-1">
          {parsed.items.map((item, idx) => {
            ...
            return (
              <div key={idx} className="flex items-start gap-2.5 text-sm sm:text-base text-neutral-800 font-medium">
                {qty ? (
                  <span className="inline-flex items-center justify-center min-w-[22px] h-5.5 px-1.5 rounded bg-amber-100 text-amber-900 border border-amber-300 font-bold text-xs shrink-0 mt-0.5">
                    {qty}
                  </span>
                ) : (
        ...
        {parsed.footerNote && (
          <div className="pt-2 border-t border-neutral-200 mt-2 flex items-center gap-2 text-xs sm:text-sm text-neutral-700 font-medium">
            <span className="text-sky-600 text-sm shrink-0">❄️</span>
            <span className="italic">{parsed.footerNote}</span>
          </div>
        )}
  ...
  if (!parsed.isList) {
    return (
      <div className={`mt-1 ${className}`}>
        ...
        {parsed.footerNote && (
          <div className="text-[10px] text-neutral-600 italic mt-1 flex items-center gap-1">
            <span className="text-sky-600 text-[10px]">❄️</span>
  ...
  return (
    <div className={`mt-1.5 space-y-0.5 ${className}`}>
      ...
      <div className="space-y-0.5 max-h-20 overflow-y-auto pr-1">
        {parsed.items.map((item, idx) => (
          <div key={idx} className="text-[11px] text-neutral-800 font-medium truncate flex items-center gap-1">
      ...
      {parsed.footerNote && (
        <div className="text-[10px] text-neutral-600 italic pt-1 border-t border-neutral-200 mt-1 flex items-center gap-1">
          <span className="text-sky-600 text-[10px]">❄️</span>
          <span className="truncate">{parsed.footerNote}</span>
        </div>
      )}
