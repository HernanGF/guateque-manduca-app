  ...
  return (
    <div className="min-h-screen bg-white text-neutral-900 pb-28">
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-neutral-200">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="p-2 rounded-lg bg-neutral-100 border border-neutral-200 text-neutral-700 hover:text-neutral-950 hover:bg-neutral-200 transition-colors"
            ...
            <div className="flex items-center gap-2.5">
              <img
                ...
                alt="Logo"
                className="w-9 h-9 rounded-full object-cover border border-neutral-200 bg-white shrink-0"
                referrerPolicy="no-referrer"
              />
              <div className="leading-tight hidden xs:block">
                <h1 className="font-extrabold text-base sm:text-lg text-neutral-900 truncate max-w-[170px] sm:max-w-xs">
                  {business.name}
                </h1>
                <span className="text-xs sm:text-sm text-neutral-500 block font-medium">Menú interactivo</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              ...
              onClick={onOpenAdmin}
              className="w-9 h-9 flex items-center justify-center rounded-xl text-neutral-400 hover:text-amber-500 hover:bg-neutral-100 border border-transparent hover:border-neutral-200 transition-all cursor-pointer"
              title="Acceso seguro"
        ...
        <div className="max-w-6xl mx-auto px-4 pb-3">
          <div className="relative w-full">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-2.5 sm:py-3 bg-neutral-50 border border-neutral-200 rounded-xl text-base text-neutral-900 placeholder:text-neutral-400 focus:outline-none focus:border-amber-500 focus:bg-white transition-colors"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute right-3.5 top-1/2 -translate-y-1/2 text-sm font-semibold text-neutral-600 hover:text-neutral-900 px-2 py-1 bg-neutral-200 rounded-md"
              >
                Limpiar
              </button>
            )}
        ...
        <div className="border-t border-neutral-200 bg-white/95 overflow-x-auto no-scrollbar py-2">
                ...
                <button
                  ...
                  className={`px-4 sm:px-5 py-2 sm:py-2.5 rounded-full text-sm sm:text-base font-bold whitespace-nowrap transition-all cursor-pointer ${
                    isActive
                      ...
                      : 'bg-neutral-100 border border-neutral-200 text-neutral-800 hover:bg-neutral-200 hover:text-neutral-950 shadow-xs'
                  }`}
                >
                  {cat.name}
                </button>
      ...
      <main className="max-w-6xl mx-auto px-4 pt-6">
            ...
            <div className="flex items-center bg-neutral-100 border border-neutral-200 rounded-xl p-1 gap-1">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs sm:text-sm transition-all flex items-center gap-1.5 cursor-pointer ${
                  viewMode === 'list'
                    ? 'bg-white text-neutral-950 font-bold shadow-xs border border-neutral-200/80'
        ...
        {filteredProducts.length === 0 && (
          <div className="py-16 text-center bg-neutral-50 border border-neutral-200 rounded-2xl p-6">
            <AlertCircle className="w-10 h-10 text-neutral-400 mx-auto mb-3" />
            <p className="text-neutral-800 font-semibold text-base">No encontramos productos</p>
            ...
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="mt-4 text-sm font-bold text-amber-600 hover:underline cursor-pointer"
              >
            ...
            {filteredProducts.map((product) => {
              ...
              return (
                <div
                  ...
                  className={`group relative bg-white border rounded-2xl p-3.5 sm:p-4 transition-all duration-200 flex items-center justify-between gap-3.5 sm:gap-4 shadow-xs ${
                    isDiscontinued
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-between self-stretch py-0.5">
                    <div>
                      ...
                      <div className="flex items-center gap-1.5 mb-2 flex-wrap">
                        {!isDiscontinued && product.isFeatured && (
                          <span className="bg-amber-500/15 text-amber-700 border border-amber-500/30 font-bold text-xs sm:text-sm px-2.5 py-0.5 rounded-full flex items-center gap-1 shadow-xs">
                            <Sparkles className="w-3.5 h-3.5" />
                            Destacado
                          </span>
                        )}
                        {product.priceType === 'variants' && !isDiscontinued && (
                          <span className="bg-neutral-100 text-neutral-700 text-xs sm:text-sm font-semibold px-2.5 py-0.5 rounded-md border border-neutral-200">
                            Con opciones
                          </span>
                        )}
                        {product.modifierGroupIds && product.modifierGroupIds.length > 0 && !isDiscontinued && (
                          <span className="bg-blue-50 text-blue-700 border border-blue-200 text-xs sm:text-sm font-semibold px-2.5 py-0.5 rounded-md flex items-center gap-1">
                      ...
                      <h3 className="font-extrabold text-lg sm:text-xl text-neutral-900 group-hover:text-amber-600 transition-colors line-clamp-2 leading-snug">
                        {product.name}
                      </h3>
                      <ProductDescription
                    ...
                    {/* Bottom Price & Button */}
                    <div className="mt-3.5 pt-2.5 border-t border-neutral-100 flex items-end justify-between gap-2">
                      <div className="min-w-0">
                        <span className="text-[11px] text-neutral-500 block uppercase tracking-wider font-bold mb-0.5">
                          Precio
                        </span>
                      ...
                      {!isDiscontinued ? (
                        <button
                          ...
                          className="shrink-0 flex items-center gap-1.5 px-3.5 sm:px-4 py-2 sm:py-2.5 bg-neutral-100 group-hover:bg-amber-500 group-hover:text-neutral-950 text-neutral-800 text-sm sm:text-base font-bold rounded-xl transition-colors cursor-pointer border border-neutral-200 group-hover:border-amber-500"
                        >
                        ...
                        <span className="text-sm sm:text-base text-neutral-400 font-medium shrink-0">Agotado</span>
                      )}
                    </div>
                  </div>

                  {/* Right: Square 1:1 image thumbnail (Shows 100% of the food, no severe crop) */}
                  <div className="relative w-28 h-28 sm:w-32 sm:h-32 md:w-36 md:h-36 rounded-xl overflow-hidden shrink-0 bg-neutral-100 border border-neutral-200 shadow-inner">
                    <ProductThumbnail
            ...
            {filteredProducts.map((product) => {
              ...
              return (
                <div
                  ...
                  className={`group relative bg-white border rounded-2xl overflow-hidden transition-all duration-200 flex flex-col justify-between shadow-xs ${
                    isDiscontinued
                      ? 'border-neutral-200 opacity-60 cursor-not-allowed'
                      : 'border-neutral-200 hover:border-neutral-300 hover:shadow-md cursor-pointer'
                  }`}
                >
                  ...
                  <div className="relative aspect-4/3 w-full bg-neutral-100 overflow-hidden">
                    ...
                    <div className="absolute bottom-2.5 left-2.5 flex items-center gap-1.5 flex-wrap">
                      {product.priceType === 'variants' && !isDiscontinued && (
                        <div className="bg-white/90 backdrop-blur-sm text-neutral-800 text-xs sm:text-sm font-semibold px-2.5 py-0.5 rounded-md border border-neutral-200 shadow-xs">
                          Con opciones
                        </div>
                      )}
                      {product.modifierGroupIds && product.modifierGroupIds.length > 0 && !isDiscontinued && (
                        <div className="bg-blue-50/90 backdrop-blur-sm text-blue-700 text-xs sm:text-sm font-semibold px-2.5 py-0.5 rounded-md border border-blue-200 flex items-center gap-1 shadow-xs">
                          <Sparkles className="w-3.5 h-3.5 text-blue-500" />
                          Salsas / Extras
                        </div>
                      )}
                  ...
                  <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between">
                    <div>
                      <h3 className="font-extrabold text-xl sm:text-2xl text-neutral-900 group-hover:text-amber-600 transition-colors line-clamp-2 leading-snug">
                        {product.name}
                      </h3>
                      <ProductDescription
                        description={product.description}
                    ...
                    <div className="mt-4 pt-3 border-t border-neutral-100 flex items-end justify-between gap-2">
                      <div className="min-w-0">
                        <span className="text-[11px] text-neutral-500 block font-bold uppercase tracking-wider mb-0.5">
                          Precio
                        </span>
                        {renderProductPrice(product)}
                      </div>
                      ...
                      {!isDiscontinued ? (
                        <button
                          ...
                          className="shrink-0 flex items-center gap-1.5 px-4 py-2.5 bg-neutral-100 hover:bg-amber-500 hover:text-neutral-950 text-neutral-800 text-sm sm:text-base font-bold rounded-xl transition-colors cursor-pointer border border-neutral-200 hover:border-amber-500"
                        >
                          <Plus className="w-4 h-4" />
                          <span>
                            {product.priceType === 'variants'
